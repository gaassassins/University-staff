...
struct edge// неорграф x — из какой вершины, y — в какую, w — вес ребра. 
{
int x, y, w;
edge(){}
...
//После того, как считали граф, отсортировываем все ребра в порядке неубывания
...
int getLeader(int x)
//узнаем, в каком подмножестве находится ребро
{
if (x == leader[x])
return x;
...
bool unite(int x, int y)//Для того, чтобы ответить на вопрос объединять ли две вершины используется функция unite. 
{
x = getLeader(x);
y = getLeader(y);
...
leader.resize(n); // сортируем ребра и узнаем, в каком множестве они находятся
...
if (unite(x, y))
ans.push_back(e[i]); // добавляем ребро в вектор
}
